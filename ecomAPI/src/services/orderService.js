    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.addressUserId || !data.typeShipId) {
                ...
                data.arrDataShopCart = data.arrDataShopCart.map(
                    (item, index) => {
        ...
        try {
            if (!id) {
                ...
                for (let i = 0; i < orderDetail.length; i++) {
                    ...
                    orderDetail[i].productDetail =
                        await db.ProductDetail.findOne({
                            where: {
                                id: orderDetail[i].productDetailSize
                                    .productdetailId,
                            },
                        });
                    ...
                    orderDetail[i].productImage = await db.ProductImage.findAll(
                        {
                            where: {
                                productdetailId:
                                    orderDetail[i].productDetail.id,
                            },
                        }
                    );
                    for (
            ...
            if (!userId) {
                ...
                for (let i = 0; i < addressUser.length; i++) {
                    ...
                    for (let j = 0; j < addressUser[i].order.length; j++) {
                        addressUser[i].order[
                            j
                        ].voucherData.typeVoucherOfVoucherData =
                            await db.TypeVoucher.findOne({
                                where: {
                                    id: addressUser[i].order[j].voucherData
                                        .typeVoucherId,
                                },
                            });
                        let orderDetail = await db.OrderDetail.findAll({
                        ...
                        for (let k = 0; k < orderDetail.length; k++) {
                            ...
                            orderDetail[k].productDetail =
                                await db.ProductDetail.findOne({
                                    where: {
                                        id: orderDetail[k].productDetailSize
                                            .productdetailId,
                                    },
                                });
                            ...
                            orderDetail[k].productImage =
                                await db.ProductImage.findAll({
                                    where: {
                                        productdetailId:
                                            orderDetail[k].productDetail.id,
                                    },
                                });
                            for (
                                let f = 0;
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let i = 0; i < data.result.length; i++) {
                data.result[i].productDetailSize =
                    await db.ProductDetailSize.findOne({
                ...
                totalPriceProduct +=
                    data.result[i].realPrice * data.result[i].quantity;
            ...
            listItem.push({
                ...
                sku: "1",
                price:
                    parseFloat(data.total - totalPriceProduct).toFixed(2) + "",
                currency: "USD",
                quantity: 1,
            });
                    ...
                    async function (error, payment) {
                        if (error) {
                            ...
                            if (res) {
                                ...
                                for (
                                    let i = 0;
                                    i < data.arrDataShopCart.length;
                                    i++
                                ) {
                                    let productDetailSize =
                                        await db.ProductDetailSize.findOne({
                                            where: {
                                                id: data.arrDataShopCart[i]
                                                    .productId,
                                            },
                                            raw: false,
                                        });
                                    productDetailSize.stock =
                                        productDetailSize.stock -
                                        data.arrDataShopCart[i].quantity;
                                    await productDetailSize.save();
                                }
                            }
                            if (data.voucherId && data.userId) {
